...
5. **Create each Subtask** — fill completely (precise scope, concrete acceptance criteria, named files, pattern to follow, explicit verification/testing). `createJiraIssue` (SCA, `Subtask`, `parent: <FEATURE-KEY>`). No raw placeholders. **Then set status to `To Do`** (`transitionJiraIssue`, id `21`) so it doesn't linger in the `Needs Visual Design` default — unless it genuinely needs design first (leave/move to `Needs Visual Design`, id `11`). See the Status-transitions table in [`../_reference.md`](../_reference.md).
