...
- Epic = top / product tier (the only issue type above base level; SCA-10 is one).
- Feature = base-level issue, parented to an Epic. Every Feature must belong to an Epic.
- Subtask = leaf implementation item, parented to a Feature.
There is NO separate "Feature Slice" tier: if a Feature is too large, split it into
multiple sibling Features under the same Epic.
THIS TEMPLATE IS FOR THE **EPIC** TIER.
-->
...
{{ Brief description of likely changes }}
...
{{ Database, table, queue, service, job, bucket, API, environment variable, etc. }}
...
Validate the likely affected areas against the current repository.

Identify unresolved blocking decisions before creating implementation tasks.

Create stories around independently testable outcomes.

Prefer vertical feature slices over file-by-file or layer-by-layer stories.

Separate infrastructure, migration, and investigation work when independently deliverable.

Reference the relevant acceptance criteria in each story.

Avoid unrelated refactoring.

Surface contradictions, missing requirements, and risky assumptions rather than inventing answers.

Include testing, observability, permissions, and failure handling where relevant.

Mark the epic complete only when its acceptance criteria have been validated.